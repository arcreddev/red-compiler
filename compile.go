...
func runmod(code string) {
	...
	case "SETINDEX":
		// Set the value of the array which is given in the first paramter at the index given in the second parameter to the value at the top of the tempstack
		val := module.symbols[parts[1]]
		if val.dtype != 4 {
			fmt.Println("Cannot index non-array")
			os.Exit(1)
		}
		...
		if index >= len(val.list) {
			...
			val.list[index] = tempstack[len(tempstack)-1]
			tempstack = tempstack[:len(tempstack)-1]
		}
	case "PRINT":
...
func run(code string) {
	...
	case "SETINDEX":
		...
		if index >= len(val.list) {
			...
			val.list[index] = stack[len(stack)-1]
			stack = stack[:len(stack)-1]
		}
	case "PRINT":
				...
				if line != "" {
					...
					case "EXPORT":
						...
						if len(partsin) > 2 {
							...
							if err != nil {
								var s stackVal = stackVal{dtype: 1, sval: string(strings.Join(partsin[2:], " "))}
...
func main() {
	...
	// Split the input into lines
	lines := strings.Split(string(bytes), "\n")
	comb := strings.Join(lines, "\n")

	res := fmt.Sprintf(rest, "`", comb, "`\n")
	f, err := os.Create(os.Args[2] + ".go")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	cmd := exec.Command("go", "build", os.Args[2]+".go")
	...
	cmd = exec.Command("rm", os.Args[2]+".go")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err = cmd.Run()
	if err != nil {
